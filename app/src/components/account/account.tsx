...
export default function Account() {
    ...
    const generateIban = () => {
        const randomNumbers = Array.from({ length: 24 }, () => Math.floor(Math.random() * 10)).join("");
        ...
        <>
            <div className="flex flex-row justify-between p-10">
                <div>
                    ...
                    <p className="text-gray-400">Total des actifs : {totalBalance.toLocaleString()} €</p>
                </div>
                <div>
                    <AddAccount />
                </div>
                            ...
                            <div className="border-solid border-2 border-black w-full h-1/3 m-auto p-5">
                                <div className="flex flex-col gap-4">
                                    <div className="flex flex-row justify-between">
                                        <p className="font-bold">{account.name ? `${account.name} €` : "Chargement..."}</p>
                                        <IconButton>
